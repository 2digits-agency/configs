## @2digits/opencode-plugin@0.2.34

### Adopt TypeScript 7 while preserving ESLint compatibility

- Migrated non-ESLint package type checks and declaration builds to TypeScript 7.0.2, replacing native-preview and alias dependencies.
- Retained fast `tsgo` type checks for the ESLint config and plugin while keeping their TypeScript compiler API dependencies on TypeScript 6.0.3 through the named `eslint` catalog.
- Expanded the shared tsconfig's TypeScript peer range to support both TypeScript 6 and 7.

## @2digits/opencode-plugin@0.2.33

### Update @opencode-ai/plugin to 1.18.29

## @2digits/opencode-plugin@0.2.32

### Update the OpenCode plugin API to 1.18.27

## @2digits/opencode-plugin@0.2.31

### Update the OpenCode plugin API to 1.18.26



### Update PostHog Node to 5.51.6

## @2digits/opencode-plugin@0.2.30

### Update `@opencode-ai/plugin` to 1.18.25



### Update `posthog-node` to 5.51.4

## @2digits/opencode-plugin@0.2.29

### Update `@opencode-ai/plugin` to 1.18.23



### Update `posthog-node` to 5.51.2

## @2digits/opencode-plugin@0.2.28

### Update `@opencode-ai/plugin` to 1.18.21



### Update `posthog-node` to 5.50.0

## @2digits/opencode-plugin@0.2.27

### Update `@opencode-ai/plugin` to 1.18.18



### Update `posthog-node` to 5.49.0



### Update `posthog-node` to 5.49.1

## @2digits/opencode-plugin@0.2.26

### Update @opencode-ai/plugin to 1.18.12



### Update posthog-node to 5.47.8



### Update @opencode-ai/plugin to 1.18.15



### Update posthog-node to 5.48.1

## @2digits/opencode-plugin@0.2.25

### Update @opencode-ai/plugin to 1.18.11



### Update posthog-node to 5.47.3

## @2digits/opencode-plugin@0.2.24

### Update @opencode-ai/plugin to 1.18.9



### Update @opencode-ai/plugin to 1.18.5



### Update posthog-node to 5.46.1

## @2digits/opencode-plugin@0.2.23

### Update OpenCode plugin dependencies

- Updated `@opencode-ai/plugin` to 1.18.4
- Updated `posthog-node` to 5.46.0

## @2digits/opencode-plugin@0.2.22

### Update OpenCode plugin dependencies

- Updated `@opencode-ai/plugin` to 1.18.3
- Updated `posthog-node` to 5.45.2

# @2digits/opencode-plugin

## 0.2.21

### Patch Changes

- bfb16bd: Update OpenCode plugin dependencies

  - Updated `@opencode-ai/plugin` to 1.18.2
  - Updated `posthog-node` to 5.44.0

## 0.2.20

### Patch Changes

- 7916d9c: Update OpenCode plugin dependencies

  - Updated '@opencode-ai/plugin' to 1.17.18.
  - Updated 'posthog-node' to 5.40.0.

## 0.2.19

### Patch Changes

- 3103077: Update opencode plugin dependencies

  - Updated `@opencode-ai/plugin` to 1.17.13
  - Updated `posthog-node` to 5.39.4

## 0.2.18

### Patch Changes

- e88c38d: Update opencode plugin dependencies

  - Updated `@opencode-ai/plugin` to 1.17.11
  - Updated `posthog-node` to 5.38.6

## 0.2.17

### Patch Changes

- 433d415: Update opencode plugin dependencies

  - Updated `@opencode-ai/plugin` to 1.17.0
  - Updated `posthog-node` to 5.36.10

## 0.2.16

### Patch Changes

- 0b1927b: Update posthog-node to 5.35.13

## 0.2.15

### Patch Changes

- 128f6a2: Update @opencode-ai/plugin to 1.15.13

  - Updated `posthog-node` to 5.35.7

## 0.2.14

### Patch Changes

- 626d181: Update @opencode-ai/plugin to 1.15.11

  - Updated `posthog-node` to 5.35.5

- cb2b172: Update @opencode-ai/plugin to 1.15.6

  - Updated `posthog-node` to 5.34.8

## 0.2.13

### Patch Changes

- 158630a: Update @opencode-ai/plugin to 1.15.5 and posthog-node to 5.34.6

## 0.2.12

### Patch Changes

- efd6d57: Update @opencode-ai/plugin to 1.15.1 and posthog-node to 5.34.2

## 0.2.11

### Patch Changes

- 62087fd: Update @opencode-ai/plugin to 1.14.50

## 0.2.10

### Patch Changes

- 1389c83: Update @opencode-ai/plugin to 1.14.49 and posthog-node to 5.34.1

  - Updated `@opencode-ai/plugin` to 1.14.49
  - Updated `posthog-node` to 5.34.1

## 0.2.9

### Patch Changes

- af1fdf7: Update opencode plugin dependencies

  - Updated `@opencode-ai/plugin` to 1.14.42
  - Updated `posthog-node` to 5.33.4

## 0.2.8

### Patch Changes

- 353439d: Update opencode plugin dependencies

  - Updated `@opencode-ai/plugin` to 1.14.40
  - Updated `posthog-node` to 5.33.3

## 0.2.7

### Patch Changes

- 5715cd2: Update opencode plugin dependencies

  - Updated `@opencode-ai/plugin` to 1.14.35
  - Updated `posthog-node` to 5.33.2

## 0.2.6

### Patch Changes

- c1eabd5: Update @opencode-ai/plugin to 1.14.33
- c1eabd5: Update posthog-node to 5.33.0
- 68924a9: Small internal performance refactors

## 0.2.5

### Patch Changes

- 3aa665b: Update posthog-node to 5.32.0
- 3aa665b: Update @opencode-ai/plugin to 1.14.31

## 0.2.4

### Patch Changes

- 1e6b41a: Update posthog-node to 5.30.8
- 1e6b41a: Update @opencode-ai/plugin to 1.14.29

## 0.2.3

### Patch Changes

- e1f712a: Update posthog-node to 5.30.6
- 18cf38c: Fix PostHog LLM trace capture

  - Sent `$ai_trace` snapshots immediately when assistant generations complete
  - Added trace input, output, and cost properties for PostHog trace summaries
  - Avoided duplicate trace snapshots when trace state has not changed

- e1f712a: Update @opencode-ai/plugin to 1.14.28
- 5e40cb5: Switch build tooling from bunup to vp pack

  - Replaced `bunup` with `vp pack` for builds
  - Removed Bun-specific dev dependencies (`@types/bun`, `bunup`)
  - Added `pack` configuration to `vite.config.ts` with export metadata generation
  - Updated dist output paths to use `.mjs` and `.d.mts` extensions

## 0.2.2

### Patch Changes

- 36cc737: Fixed posthog api key issues
- e139dc7: Update posthog-node to 5.30.1

## 0.2.1

### Patch Changes

- 042aade: Fixed package.json main entrypoint

## 0.2.0

### Minor Changes

- ba7d21a: Refactor plugin to PostHog LLM analytics integration

  - Replaced rules-based feedback system with PostHog LLM trace and generation capture
  - Added `capture.ts`, `config.ts`, `state.ts`, `types.ts`, and `utilities.ts` for analytics pipeline
  - Removed `/fix` command, `create_feedback_issue` tool, and markdown rule injection
  - Removed `dedent` dependency and added `posthog-node` for analytics ingestion
  - Added test coverage for capture, config, state, and utilities modules
  - Updated `@opencode-ai/plugin` to 1.14.22

## 0.1.26

### Patch Changes

- 44b9d5a: Update @opencode-ai/plugin to 1.14.21

## 0.1.25

### Patch Changes

- 8837219: Update @opencode-ai/plugin to 1.14.19

## 0.1.24

### Patch Changes

- 211e03f: Update @opencode-ai/plugin to 1.14.18

## 0.1.23

### Patch Changes

- 9e37180: Update @opencode-ai/plugin to 1.4.6

## 0.1.22

### Patch Changes

- 8be7864: Update @opencode-ai/plugin to 1.4.3

## 0.1.21

### Patch Changes

- 692a9b0: Update @opencode-ai/plugin to 1.4.0

## 0.1.20

### Patch Changes

- 00e6c97: Update @opencode-ai/plugin to 1.3.17

## 0.1.19

### Patch Changes

- 43d3071: Update @opencode-ai/plugin to 1.3.12
- 2cef142: Update @opencode-ai/plugin to 1.3.11, eslint-plugin-turbo to 2.9.2, and renovate to 43.102.0

  - Update @opencode-ai/plugin to 1.3.11
  - Update eslint-plugin-turbo to 2.9.2
  - Update renovate to 43.102.0

- 43d3071: Update @opencode-ai/plugin to 1.3.13

## 0.1.18

### Patch Changes

- e5934bc: Update @opencode-ai/plugin to 1.3.10 and renovate to 43.101.5

  - Update @opencode-ai/plugin to 1.3.10
  - Update renovate to 43.101.5

## 0.1.17

### Patch Changes

- 0fc6b85: Update @opencode-ai/plugin to 1.3.9
- 2d85163: Update @opencode-ai/plugin to 1.3.5
- 2d85163: Update @opencode-ai/plugin to 1.3.7

## 0.1.16

### Patch Changes

- cd7f884: Update @opencode-ai/plugin to 1.3.3

## 0.1.15

### Patch Changes

- d26a02d: Update @opencode-ai/plugin to 1.3.2

## 0.1.14

### Patch Changes

- 85ba774: Update TypeScript to 6.0.2
- 85ba774: Update @opencode-ai/plugin to 1.3.0

## 0.1.13

### Patch Changes

- cddb192: Update @types/bun to 1.3.11

## 0.1.12

### Patch Changes

- 180fa00: Update @opencode-ai/plugin to 1.2.27

## 0.1.11

### Patch Changes

- b6625c2: Update @opencode-ai/plugin to 1.2.26

## 0.1.10

### Patch Changes

- 175947e: Update @opencode-ai/plugin to 1.2.25

## 0.1.9

### Patch Changes

- 2ea465f: Update @opencode-ai/plugin to 1.2.24

## 0.1.8

### Patch Changes

- 7355a63: Update @opencode-ai/plugin to 1.2.20

## 0.1.7

### Patch Changes

- f446e43: Update @opencode-ai/plugin to 1.2.18

## 0.1.6

### Patch Changes

- b8fce21: Update @opencode-ai/plugin to 1.2.16

## 0.1.5

### Patch Changes

- 0eaa299: Update `@types/bun` to 1.3.10

## 0.1.4

### Patch Changes

- 007a0e9: Update @opencode-ai/plugin to 1.2.15

## 0.1.3

### Patch Changes

- d22e0ed: Update @opencode-ai/plugin to 1.2.12

## 0.1.2

### Patch Changes

- d643f5d: Update @opencode-ai/plugin to 1.2.10

## 0.1.1

### Patch Changes

- a33b04c: Update @opencode-ai/plugin to 1.2.6

  - Updated `@opencode-ai/plugin` from `1.2.5` to `1.2.6`

- f7712d5: Update @opencode-ai/plugin to 1.2.5

## 0.1.0

### Minor Changes

- 590eea0: Update @opencode-ai/plugin to 1.2.4

### Patch Changes

- 834baf1: Update @opencode-ai/plugin to 1.1.60

## 0.0.12

### Patch Changes

- 13454b5: Update @opencode-ai/plugin to 1.1.54

## 0.0.11

### Patch Changes

- 0877a3c: Update @opencode-ai/plugin to 1.1.53

## 0.0.10

### Patch Changes

- 887ec1f: Update @opencode-ai/plugin to 1.1.49

## 0.0.9

### Patch Changes

- efadf20: Update @opencode-ai/plugin to 1.1.39
- 7fa2e59: Update @opencode-ai/plugin to 1.1.42
- 8be246e: Update @opencode-ai/plugin to 1.1.48

## 0.0.8

### Patch Changes

- 71c96f8: Update @opencode-ai/plugin to 1.1.36

## 0.0.7

### Patch Changes

- 6aec756: Update @opencode-ai/plugin to 1.1.35

## 0.0.6

### Patch Changes

- 7b6ba6d: Refactor feedback tool and add coding style rules
  - Extracted `createFeedbackIssue` tool to separate file with improved arg descriptions using `.meta()`
  - Added OpenCode version and project context to feedback issue body
  - Added "Method Notation in Object Literals" rule preferring `name() {}` over `name: () => {}`
  - Added "Embedded Language Strings" rule for using `dedent` with language-named imports for syntax highlighting

## 0.0.5

### Patch Changes

- 91ee064: Update @opencode-ai/plugin to 1.1.28

## 0.0.4

### Patch Changes

- ba8d11a: Update @opencode-ai/plugin to 1.1.27

## 0.0.3

### Patch Changes

- 7625383: Update @typescript/native-preview to 7.0.0-dev.20260120.1

## 0.0.2

### Patch Changes

- 730b63f: Update @opencode-ai/plugin to 1.1.26

## 0.0.1

### Patch Changes

- dc1f039: Add OpenCode plugin for shared team rules and feedback system
  - Created new package providing OpenCode plugin hooks for 2digits team
  - Added `/fix` command to submit feedback about agent behavior via GitHub issues
  - Included shared rules for code quality, React, and Effect patterns via markdown files
  - Integrated `create_feedback_issue` tool to create labeled GitHub issues in 2digits-agency/configs
